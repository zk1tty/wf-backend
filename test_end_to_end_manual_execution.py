...
SESSION_TOKEN = "eyJhbGciOiJIUzI1NiIsImtpZCI6IklKclRKS29xc0Z6ZmxUVWMiLCJ0eXAiOiJKV1QifQ..-jrwDhOMtvfHKhoeSgU5jL91PC6eTo1sdosnu5yTbx0"
...
class RRWebEventMonitor:
            ...
            async with websockets.connect(stream_url) as websocket:
                ...
                while time.time() - start_time < duration:
                    try:
                        ...
                        print(f"📡 Received rrweb event: {event_data.get('event_data', {}).get('type', 'unknown')} at {time.time()}")
    ...
    def get_event_summary(self):
        ...
        for event in self.events:
            event_type = event['data'].get('event_data', {}).get('type', 'unknown')
...
async def test_end_to_end_manual_execution():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            # Step 4: Monitor session status more aggressively during execution
            print(f"\n👀 Step 4: Monitoring session status during execution...")
            session_ready = False
            session_was_ready_at_some_point = False
            
            # Check status more frequently and for longer during execution window
            for attempt in range(1, 25):  # Check for 50 seconds total
                print(f"🔍 Attempt {attempt}/25 - Checking session status...")
                ...
                async with session.get(f"{BASE_URL}/workflows/visual/{session_id}/status", headers=headers) as resp:
                    if resp.status == 200:
                        ...
                        print(f"   🔧 Streaming Ready: {streaming_ready}, Active: {streaming_active}, Browser Ready: {browser_ready}, Events: {events_processed}")
                        
                        if streaming_ready and browser_ready:
                            print(f"   ✅ Session is ready for streaming!")
                            session_ready = True
                            session_was_ready_at_some_point = True
                            break
                        elif streaming_ready or browser_ready:
                            # Session was ready at some point
                            session_was_ready_at_some_point = True
            ...
            # Step 7: Final status check (informational only)
            print(f"\n🏁 Step 7: Final status check (informational)...")
            final_status = await monitor_session_status(session, session_id, max_attempts=1)
            print(f"   ℹ️ Final session status: {'Ready' if final_status else 'Not Ready (normal after completion)'}")
            ...
            success_criteria = {
                ...
                "session_ready": session_was_ready_at_some_point or event_summary['total_events'] > 0  # Consider successful if events were received
            }
